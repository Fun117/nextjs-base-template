...
// React Icons
import { FaDiscord, FaGithub, FaReddit, FaTwitter, FaYoutube } from "react-icons/fa";
            ...
            <footer className="border-t flex flex-col w-full transition-all duration-100 ease-in-out">
                <div className="flex justify-between flex-row flex-wrap px-5 md:px-20 py-5 md:py-10">
                    <div className="flex flex-col gap-4 mr-5">
                        <Link href={_config.author.url || `/`} target="_block" className="flex items-center">
                            {_config.footer?.ui.logo?.url &&
                                ...
                                width={40}
                                height={40}
                                className={`text-[11px] w-[40px] h-[40px] hover:scale-[102%] hover:shadow-lg active:scale-[98%] active:shadow transition-all duration-300 ease-in-out ${_config.footer?.ui.logo?.css || ''}`}
                                />
                            }
                            {_config.footer?.ui.label &&
                                <p className="font-bold text-base ml-2">{t(_config.footer?.ui.label)}</p>
                            }
                        </Link>
                        <div className="flex gap-2">
                            {_config.author.socialAccounts?.github &&
                                <Link aria-label="github" href={_config.author.socialAccounts?.github} target="_block" className="border-neutral-300/40 p-[6px] border rounded-lg shadow-sm hover:shadow-lg hover:scale-[102%] active:shadow active:scale-[95%] transition-all duration-300 ease-in-out">
                                    <FaGithub className="text-xl"/>
                                </Link>
                            }
                            {_config.author.socialAccounts?.twitter &&
                                <Link aria-label="twitter" href={_config.author.socialAccounts?.twitter} target="_block" className="border-neutral-300/40 p-[6px] border rounded-lg shadow-sm hover:shadow-lg hover:scale-[102%] active:shadow active:scale-[95%] transition-all duration-300 ease-in-out">
                                    <FaTwitter className="text-xl"/>
                                </Link>
                            }
                            {_config.author.socialAccounts?.youtube &&
                                <Link aria-label="youtube" href={_config.author.socialAccounts?.youtube} target="_block" className="border-neutral-300/40 p-[6px] border rounded-lg shadow-sm hover:shadow-lg hover:scale-[102%] active:shadow active:scale-[95%] transition-all duration-300 ease-in-out">
                                    <FaYoutube className="text-xl"/>
                                </Link>
                            }
                            {_config.author.socialAccounts?.reddit &&
                                <Link aria-label="reddit" href={_config.author.socialAccounts?.reddit} target="_block" className="border-neutral-300/40 p-[6px] border rounded-lg shadow-sm hover:shadow-lg hover:scale-[102%] active:shadow active:scale-[95%] transition-all duration-300 ease-in-out">
                                    <FaReddit className="text-xl"/>
                                </Link>
                            }
                            {_config.author.socialAccounts?.discord &&
                                <Link aria-label="discord" href={_config.author.socialAccounts?.discord} target="_block" className="border-neutral-300/40 p-[6px] border rounded-lg shadow-sm hover:shadow-lg hover:scale-[102%] active:shadow active:scale-[95%] transition-all duration-300 ease-in-out">
                                    <FaDiscord className="text-xl"/>
                                </Link>
                            }
                            {_config.author.socialAccounts?.email &&
                                <Link aria-label="email" href={_config.author.socialAccounts?.email} target="_block" className="border-neutral-300/40 p-[6px] border rounded-lg shadow-sm hover:shadow-lg hover:scale-[102%] active:shadow active:scale-[95%] transition-all duration-300 ease-in-out">
                                ...
                                <nav key={index} className="flex flex-col gap-4">
                                    ...
                                    <ul className="flex flex-col gap-2">
                                        {group.links.map((content, index2: number) => {
                                            return (
                                                <li key={index2}>
                                                    <Link href={content.url} target={content.target} className="hover:underline">
                                                    {t(content.label)}
                                                    </Link>
                                                </li>
                                            );
                ...
                <div className="border-neutral-200 border-t flex items-center justify-center w-auto p-3 mx-[5%]">
                    <span>
                    &copy; {_config.year}{` `}{t(`site:all-rights-reserved`)}
                    </span>
                </div>
            </footer>
        </div>
